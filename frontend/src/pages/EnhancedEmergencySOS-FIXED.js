...
import { AlertTriangle, MapPin, Phone, Hospital, Shield, Zap, CheckCircle, Clock, Wifi, WifiOff } from 'lucide-react';
...
const EnhancedEmergencySOS = () => {
  ...
  const sendEmergencyAlert = async () => {
    ...
    try {
      if (isOnline) {
        ...
        await Promise.all([
          sosAPI.notifyPolice(alertData).catch(err => console.error('Police notification failed:', err)),
          sosAPI.notifyHospitals(alertData).catch(err => console.error('Hospital notification failed:', err)),
          sosAPI.notifyRescueTeams(alertData).catch(err => console.error('Rescue team notification failed:', err))
        ]);
  ...
  const retryOfflineAlerts = async () => {
    ...
    for (const alert of offlineQueue) {
      try {
        await sosAPI.sendAlert(alert);
        await Promise.all([
          sosAPI.notifyPolice(alert),
          sosAPI.notifyHospitals(alert),
          sosAPI.notifyRescueTeams(alert)
        ]);
      } catch (err) {
